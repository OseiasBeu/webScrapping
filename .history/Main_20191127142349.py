...
query = 
SELECT 
pedido.clienteEstado, pedidoItem.warehouseId, count(pedidoItem.warehouseId) as "Pendentes de integração" FROM pedido LEFT JOIN pedidoItem ON pedido.codigoPedido = pedidoItem.codigoPedido WHERE pedido.datahoracriacao > '2019-11-01' AND pedido.clientepais = 'BRA' AND pedido.flagIntegrado = 0 GROUP BY pedidoItem.warehouseId, pedido.clienteEstado ORDER BY "Pendentes de integração" DESC
