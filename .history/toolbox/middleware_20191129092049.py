...
def middleware():
  ...
  df1['timeStamp'] = datetime.now().strftime('%m/%d/%y %H:%M:%S')
  ...
  df1.append(base_m, ignore_index=True)
  # pd.concat([df1]*3, ignore_index=True)
  df_repeat = pd.concat([df1,base_m], sort=False)
  print(df_repeat)
  
  
  nomeArquivo = 'base_middleware.xlsx'
  df_repeat.to_excel(nomeArquivo, index=False)
