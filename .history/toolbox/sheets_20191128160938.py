...
def insertPlanIntegracoes_h(rows):
    ...
    request = service.spreadsheets().values().update(spreadsheetId=SAMPLE_SPREADSHEET_ID, range=range_, valueInputOption=value_input_option, body=value_range_body, includeValuesInResponse = include_Values_In_Response)response = request.execute()
    print(response)
    log = open("logs/logs.txt", "a+")
    log.write('\n------------------------------------------------------\n')
    log.write('\nLinhas inseridas com sucesso!\n')
