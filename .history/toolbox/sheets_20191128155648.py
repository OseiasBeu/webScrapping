...
def insertPlanMiddleware(rows):
    ...
    for row in linhas:    
        ...
        p3 = row[3]   
        uma_linha.append(p3)
        uma_linha[0] = str(datetime.now()) # pega o tempo mais perto de inclusão
        uma_linha.append(p0)
        uma_linha.append(p1)
        uma_linha.append(p2)
...
def insertPlanIntegracoes_h(rows):
    ...
    range_ = 'QA_integracoes_h!A2'  # TODO: Update placeholder value.
    ...
    request = service.spreadsheets().values().update(spreadsheetId=SAMPLE_SPREADSHEET_ID, range=rangeClear, valueInputOption=value_input_option, body=value_range_body, includeValuesInResponse = include_Values_In_Response)
    response = request.execute()
    print(response)
    log = open("logs/logs.txt", "a+")
    log.write('\n------------------------------------------------------\n')
