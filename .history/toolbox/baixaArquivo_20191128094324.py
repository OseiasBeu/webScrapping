...
def baixaArquivo():
  ...
  print('coluna final{}: '.format(coluna))
  print(arr[4:])
  
  df = pd.DataFrame(data=arr[4:],index=None, columns=coluna)
  print(df)
  df['timeStamp'] = datetime.now()
  df = df.reindex[['timeStamp','clienteEstado','warehouseId','Pendentes de integração']]
  print(df)
