...
def middleware():
  ...
  bra = "BRA"
  data = '2019-11-01' 
  query = "SELECT Cast(Day(datahoraprocessamento) AS VARCHAR(2)) + '/'        + Cast(Month(datahoraprocessamento) AS VARCHAR(2)) 
       + '/' 
       + Cast(Year(datahoraprocessamento) AS VARCHAR(4)) AS Data, 
       Datepart(hour, datahoraprocessamento)             AS Hora, 
