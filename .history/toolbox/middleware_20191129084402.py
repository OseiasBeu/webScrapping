...
def middleware():
  ...
  df1['timeStamp'] = datetime.now().strftime('%m/%S/%Y %H:%M:%S')
  print(df1)
  antes = np.asarray(df1)
  wb = load_workbook("base_middleware.xlsx")
  # base_m = wb.active
  depois = wb.worksheets[0]

# Append 2 new Rows - Columns A - D
  cabecalho = []
  for row_data in antes:
    cabecalho.append(row_data[0])
    cabecalho.append(row_data[1])
    cabecalho.append(row_data[2])
    cabecalho.append(row_data[3])
    
    # Append Row Values
    print("entrou")

    print('row data{}'.format(cabecalho))
  depois.append(cabecalho)
  new_df = pd.DataFrame(depois)
  
  nomeArquivo = 'base_middleware.xlsx'
  new_df.to_excel(nomeArquivo, index=False)
  

  # depois.save("base_middleware.xlsx")
  # print(df1)
  # antes = np.asarray(df1)
